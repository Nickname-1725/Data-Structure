...
void treePreOrderVisit(Tree *root)
{
    if (root == NULL)
		return;
	...
	treePreOrderVisit(root->child);
	
	if(root->sibling)	
	{	
		treePreOrderVisit(root->sibling);
	}
}

void treePostOrderVisit(Tree *root)
{
    if (root == NULL)
        return;
	
	treePostOrderVisit(root->child);
